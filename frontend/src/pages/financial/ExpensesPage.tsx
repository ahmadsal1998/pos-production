...
const MOCK_EXPENSES: Expense[] = [
    {
        ...
        responsiblePerson: 'قسم الموارد البشرية',
        ...
        updatedAt: new Date(new Date().setDate(1)).toISOString()
    },
    {
        id: `EXP-${UUID()}`,
        ...
        updatedAt: new Date(new Date().setDate(5)).toISOString()
    },
    {
        id: `EXP-${UUID()}`,
        ...
        createdAt: new Date(new Date().setDate(15)).toISOString(),
        updatedAt: new Date(new Date().setDate(15)).toISOString()
    },
    {
        id: `EXP-${UUID()}`,
        ...
        createdAt: new Date(new Date().setDate(20)).toISOString(),
        updatedAt: new Date(new Date().setDate(20)).toISOString()
    }
];
...
const ExpenseFormModal: React.FC<{
  ...
  expenseToEdit: Expense | null;
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.responsiblePerson || !formData.expenseNumber || !formData.description || formData.amount <= 0) {
            alert('يرجى ملء جميع الحقول المطلوبة بمبالغ صحيحة.');
            return;
        }
        const now = new Date().toISOString();
        onSave({ 
            id: expenseToEdit?.id || `EXP-${UUID()}`, 
            ...formData,
            responsible: formData.responsiblePerson,
            createdAt: expenseToEdit?.createdAt || now,
            updatedAt: now
        });
    };
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg text-right" onClick={e => e.stopPropagation()}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{AR_LABELS.expenseNumber}</label>
                            <input type="text" name="expenseNumber" value={formData.expenseNumber} onChange={handleChange} className="w-full border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-md shadow-sm" required />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="responsiblePerson" value={formData.responsiblePerson} onChange={handleChange} className="w-full border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-md shadow-sm" required />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{AR_LABELS.description}</label>
                            <input type="text" name="description" value={formData.description} onChange={handleChange} className="w-full border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-md shadow-sm" required />
                        </div>
                        <div>
    ...
    const filteredExpenses = useMemo(() => {
        return expenses.filter(e => {
            ...
            const matchesSearch = searchTerm ?
                ...
                e.responsiblePerson.toLowerCase().includes(lowerSearch) ||
