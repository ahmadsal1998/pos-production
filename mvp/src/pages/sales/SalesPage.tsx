        ...
        <div className="relative min-h-screen overflow-hidden">
            ...
            <div className="relative mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                    <MetricCard id={1} title={AR_LABELS.totalSalesToday} value={`${summaryMetrics.totalSalesToday.toFixed(2)} ر.س`} icon={<div className="w-6 h-6 bg-green-500 rounded"></div>} bgColor="bg-green-100" valueColor="text-green-600" />
                    <MetricCard id={2} title={AR_LABELS.totalPayments} value={`${summaryMetrics.totalPayments.toFixed(2)} ر.س`} icon={<div className="w-6 h-6 bg-blue-500 rounded"></div>} bgColor="bg-blue-100" valueColor="text-blue-600" />
                    <MetricCard id={3} title={AR_LABELS.creditSales} value={`${summaryMetrics.creditSales.toFixed(2)} ر.س`} icon={<div className="w-6 h-6 bg-yellow-500 rounded"></div>} bgColor="bg-yellow-100" valueColor="text-yellow-600" />
                    <MetricCard id={4} title={AR_LABELS.invoiceCount} value={summaryMetrics.invoiceCount.toString()} icon={<div className="w-6 h-6 bg-purple-500 rounded"></div>} bgColor="bg-purple-100" valueColor="text-purple-600" />
                    <MetricCard id={5} title={AR_LABELS.collectionRate} value={`${summaryMetrics.collectionRate.toFixed(1)}%`} icon={<div className="w-6 h-6 bg-indigo-500 rounded"></div>} bgColor="bg-indigo-100" valueColor="text-indigo-600" />
                </div>
...
const SalesTableRow: React.FC<{sale: SaleTransaction, onView: (s: SaleTransaction) => void}> = ({sale, onView}) => {
    ...
    return (
        <tr className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
            ...
            <td className="px-6 py-4 whitespace-nowrap text-sm text-red-600">{sale.remainingAmount.toFixed(2)} ر.س</td>
