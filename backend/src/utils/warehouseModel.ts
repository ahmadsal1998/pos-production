...
// Warehouse document interface (without storeId since we use separate collections)
...
// Warehouse schema definition (without storeId since we use separate collections)
...
warehouseSchema.index({ createdAt: -1 }); // For sorting by creation date
warehouseSchema.index({ status: 1 }); // For filtering by status
...
/**
 * Get a Mongoose model for a store-specific Warehouse collection
 * @param prefix - Store prefix (e.g., 'store1')
 * @param databaseId - Database ID where the collection is stored
 * @returns Mongoose model for the store-specific Warehouse collection
 */
export async function getWarehouseModel(
  prefix: string | null,
  databaseId?: number
): Promise<Model<WarehouseDocument>> {
  if (!prefix) {
    // Return default model for system warehouses (if needed) - use main connection
    const defaultCollectionName = 'warehouses';
    if (mongoose.models[defaultCollectionName]) {
      return mongoose.models[defaultCollectionName] as Model<WarehouseDocument>;
    }
    return mongoose.model<WarehouseDocument>(defaultCollectionName, warehouseSchema, defaultCollectionName);
  }
  ...
  const collectionName = `${sanitizedPrefix}_warehouses`;
  ...
  if (connection.models[collectionName]) {
    return connection.models[collectionName] as Model<WarehouseDocument>;
  }

  // Create new model with the collection name on the specific connection
  return connection.model<WarehouseDocument>(collectionName, warehouseSchema, collectionName);
}

/**
 * Get Warehouse model for a user's store
 * @param storeId - User's storeId
 * @returns Mongoose model for the user's store Warehouse collection
 */
export async function getWarehouseModelForStore(storeId: string | null | undefined): Promise<Model<WarehouseDocument>> {
  ...
  return getWarehouseModel(prefix, databaseId);
}
