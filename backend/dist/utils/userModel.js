...
async function findUserAcrossStores(query, storeIdHint) {
  ...
  if (targetStoreId) {
    try {
      ...
      if (user) {
        if (user.email) (0, import_storeUserCache.cacheEmailToStore)(user.email, user.storeId);
        if (user.username) (0, import_storeUserCache.cacheUsernameToStore)(user.username, user.storeId);
        return user;
      }
    } catch (error) {
