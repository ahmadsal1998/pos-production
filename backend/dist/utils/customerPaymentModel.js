...
async function getStorePrefix(storeId) {
    try {
        ...
        logger_1.log.error(`Error getting store prefix for ${storeId}`, error);
...
async function getCustomerPaymentModel(prefix, databaseId) {
    ...
    if (connectionDbName && connectionDbName !== (0, databaseManager_1.getDatabaseName)(dbId)) {
        logger_1.log.warn(`Database name mismatch: Expected ${(0, databaseManager_1.getDatabaseName)(dbId)}, got ${connectionDbName}`);
    }
