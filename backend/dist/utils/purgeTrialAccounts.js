...
exports.getTrialAccounts = getTrialAccounts;
exports.generatePurgeReport = generatePurgeReport;
exports.purgeTrialAccounts = purgeTrialAccounts;
exports.purgeSpecificTrialAccount = purgeSpecificTrialAccount;
...
async function getTrialAccounts() {
    return await Store_1.default.find({ isTrialAccount: true }).lean();
}
/**
...
async function purgeTrialAccounts(dryRun = true, confirm = false) {
    ...
    try {
        const deleteResult = await Store_1.default.deleteMany({ isTrialAccount: true });
    ...
    try {
        const { clearTrialStatusCache } = await Promise.resolve().then(() => __importStar(require('./trialAccountModels')));
...
async function purgeSpecificTrialAccount(storeId, dryRun = true, confirm = false) {
    ...
    try {
        await Store_1.default.deleteOne({ _id: store._id });
