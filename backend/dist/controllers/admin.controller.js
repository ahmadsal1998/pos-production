...
__export(admin_controller_exports, {
  ...
  updatePointsSettings: () => updatePointsSettings,
  updateSetting: () => updateSetting,
  updateStore: () => updateStore,
  validateCreateStore: () => validateCreateStore,
  validatePointsSettings: () => validatePointsSettings,
...
var import_StoreType = __toESM(require("../models/StoreType"));
...
const getStores = (0, import_error.asyncHandler)(
  async (req, res, next) => {
    const stores = await import_Store.default.find().sort({ storeNumber: 1 }).populate("storeTypeId", "name description");
    res.status(200).json({
      success: true,
      data: {
        stores
...
const getStore = (0, import_error.asyncHandler)(
  async (req, res, next) => {
    const { id } = req.params;
    const store = await import_Store.default.findById(id).populate("storeTypeId", "name description");
    if (!store) {
...
const createStore = (0, import_error.asyncHandler)(
  async (req, res, next) => {
    ...
    const typeExists = await import_StoreType.default.findById(storeTypeId);
    if (!typeExists) {
    ...
    import_logger.log.info(`Assigning new store "${name}" to database ${databaseId}`);
    ...
    import_logger.log.info("Store created. Collections will be created automatically when first documents are inserted.");
    ...
    if (createDefaultAdmin && defaultAdminEmail && defaultAdminPassword) {
      try {
        ...
        if (error.code === 11e3 && error.keyPattern?.email) {
          ...
          return next(new import_error.AppError("Email already in use", 400));
        }
        next(error);
      }
    }
...
const updateStore = (0, import_error.asyncHandler)(
  async (req, res, next) => {
    ...
    if (storeTypeId !== void 0) {
      const st = await import_StoreType.default.findById(storeTypeId);
...
const validateCreateStore = [
  ...
  (0, import_express_validator.body)("storeTypeId").notEmpty().withMessage("Store type is required").isMongoId().withMessage("Invalid store type"),
...
const validateUpdateStore = [
  ...
  (0, import_express_validator.body)("storeTypeId").optional().isMongoId().withMessage("Invalid store type")
];
...
const getPointsSettings = (0, import_error.asyncHandler)(
  async (req, res, next) => {
...
const updatePointsSettings = (0, import_error.asyncHandler)(
  async (req, res, next) => {
    const errors = (0, import_express_validator.validationResult)(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    const settings = await import_PointsSettings.default.findOneAndUpdate(
      { storeId: targetStoreId },
      {
        userPointsPercentage,
        companyProfitPercentage,
        defaultThreshold,
        pointsExpirationDays: pointsExpirationDays || void 0,
        minPurchaseAmount: minPurchaseAmount || void 0,
        maxPointsPerTransaction: maxPointsPerTransaction || void 0,
        pointsValuePerPoint: pointsValuePerPoint || void 0
      },
      {
        new: true,
        upsert: true,
...
const validatePointsSettings = [
  (0, import_express_validator.body)("storeId").optional().trim().toLowerCase(),
  (0, import_express_validator.body)("userPointsPercentage").optional().isFloat({ min: 0, max: 100 }).withMessage("User points percentage must be between 0 and 100"),
  (0, import_express_validator.body)("companyProfitPercentage").optional().isFloat({ min: 0, max: 100 }).withMessage("Company profit percentage must be between 0 and 100"),
  (0, import_express_validator.body)("defaultThreshold").optional().isFloat({ min: 0 }).withMessage("Default threshold must be a non-negative number"),
  (0, import_express_validator.body)("pointsExpirationDays").optional().isInt({ min: 1 }).withMessage("Points expiration days must be a positive integer"),
  (0, import_express_validator.body)("minPurchaseAmount").optional().isFloat({ min: 0 }).withMessage("Minimum purchase amount must be a non-negative number"),
  (0, import_express_validator.body)("maxPointsPerTransaction").optional().isInt({ min: 1 }).withMessage("Maximum points per transaction must be a positive integer"),
  (0, import_express_validator.body)("pointsValuePerPoint").optional().isFloat({ min: 0 }).withMessage("Points value per point must be a non-negative number")
];
