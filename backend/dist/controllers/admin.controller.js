...
const createStore = (0, import_error.asyncHandler)(
  async (req, res, next) => {
    ...
    try {
      await (0, import_storeCollections.createStoreCollections)(prefix.toLowerCase(), databaseId, store.storeId.toLowerCase());
    ...
    if (createDefaultAdmin && defaultAdminEmail && defaultAdminPassword) {
      try {
        ...
        const existingUser = await (0, import_userModel.findUserAcrossStores)({
          $or: [
        ...
        if (!existingUser) {
          const userModel = await (0, import_userModel.getUserModel)(store.storeId.toLowerCase());
