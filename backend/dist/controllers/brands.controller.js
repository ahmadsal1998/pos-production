...
exports.importBrands = exports.exportBrands = exports.getBrands = exports.createBrand = exports.validateCreateBrand = void 0;
...
exports.createBrand = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    logger_1.log.debug('Create Brand - User info from token', {
    ...
    if (!storeId && req.user?.userId && req.user.userId !== 'admin') {
        try {
            ...
            if (user && user.storeId) {
                ...
                logger_1.log.debug('Create Brand - Found storeId from user record', { storeId });
            }
        }
        catch (error) {
            logger_1.log.error('Create Brand - Error fetching user', error);
        }
    }
    // Store users must have a storeId
    if (!storeId) {
        logger_1.log.warn('Create Brand - No storeId found for user');
        return res.status(400).json({
            success: false,
    ...
    try {
        ...
        logger_1.log.debug('Create Brand - Brand created successfully', { brandId: brand._id });
        res.status(201).json({
            success: true,
        ...
        logger_1.log.error('Create Brand - Error', error, { storeId });
...
exports.getBrands = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        const brands = await Brand_1.default.find({ storeId: normalizedStoreId }).sort({ createdAt: -1 });
        res.status(200).json({
            success: true,
        ...
        logger_1.log.error('Error fetching brands', error);
        return res.status(500).json({
            success: false,
...
exports.exportBrands = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    const storeId = req.user?.storeId || null;
    // Store users must have a storeId
    if (!storeId) {
        return res.status(400).json({
    ...
    try {
        ...
        logger_1.log.error('Error exporting brands', error);
        return res.status(500).json({
            success: false,
...
exports.importBrands = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    const file = req.file;
    const storeId = req.user?.storeId || null;
    if (!file) {
        return res.status(400).json({
    ...
    try {
        for (let index = 0; index < normalizedRecords.length; index += 1) {
            ...
            let existing = await Brand_1.default.findOne({
                storeId: normalizedStoreId,
                name
            });
            if (!existing) {
                existing = await Brand_1.default.findOne({
                    storeId: normalizedStoreId,
                    name: {
        ...
        logger_1.log.error('Error importing brands', error);
        return res.status(500).json({
            success: false,
