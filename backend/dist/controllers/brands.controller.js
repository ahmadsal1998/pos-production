...
const createBrand = (0, import_error.asyncHandler)(async (req, res) => {
  ...
  import_logger.log.debug("Create Brand - User info from token", {
  ...
  if (!storeId && req.user?.userId && req.user.userId !== "admin") {
    try {
      ...
      if (user && user.storeId) {
        ...
        import_logger.log.debug("Create Brand - Found storeId from user record", { storeId });
      }
    } catch (error) {
      import_logger.log.error("Create Brand - Error fetching user", error);
    }
  }
  if (!storeId) {
    import_logger.log.warn("Create Brand - No storeId found for user");
    return res.status(400).json({
      success: false,
  ...
  try {
    ...
    import_logger.log.debug("Create Brand - Brand created successfully", { brandId: brand._id });
    res.status(201).json({
      success: true,
