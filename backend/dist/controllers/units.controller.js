...
const createUnit = (0, import_error.asyncHandler)(async (req, res, next) => {
  ...
  import_logger.log.debug("Create Unit - User info from token", {
  ...
  if (!storeId && req.user?.userId && req.user.userId !== "admin") {
    try {
      ...
      if (user && user.storeId) {
        ...
        import_logger.log.debug("Create Unit - Found storeId from user record", { storeId });
      }
    } catch (error) {
      import_logger.log.error("Create Unit - Error fetching user", error);
    }
  }
  if (!storeId) {
...
const getUnits = (0, import_error.asyncHandler)(async (req, res, next) => {
...
const getUnitById = (0, import_error.asyncHandler)(async (req, res, next) => {
  const { id } = req.params;
  const storeId = req.user?.storeId || null;
  if (!storeId) {
    return res.status(400).json({
...
const updateUnit = (0, import_error.asyncHandler)(async (req, res, next) => {
  const errors = (0, import_express_validator.validationResult)(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({
      success: false,
...
const deleteUnit = (0, import_error.asyncHandler)(async (req, res, next) => {
  const { id } = req.params;
  const storeId = req.user?.storeId || null;
  if (!storeId) {
    return res.status(400).json({
...
const exportUnits = (0, import_error.asyncHandler)(async (req, res, next) => {
  const storeId = req.user?.storeId || null;
  if (!storeId) {
    return res.status(400).json({
      success: false,
...
const importUnits = (0, import_error.asyncHandler)(async (req, res, next) => {
  const file = req.file;
  const storeId = req.user?.storeId || null;
  if (!file) {
    return res.status(400).json({
