...
exports.getProductByBarcode = exports.getProductMetrics = exports.importProducts = exports.upload = exports.deleteProduct = exports.updateProduct = exports.getProduct = exports.getProducts = exports.createProduct = exports.validateCreateProduct = void 0;
...
exports.createProduct = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    const { name, barcode, costPrice, price, stock = 0, warehouseId, categoryId, brandId, description, lowStockAlert, internalSKU, vatPercentage = 0, vatInclusive = false, productionDate, expiryDate, batchNumber, discountRules, wholesalePrice, units, multiWarehouseDistribution, status = 'active', showInQuickProducts = false, } = req.body;
    ...
    try {
        ...
        if (warehouseId)
            productData.warehouseId = warehouseId.trim();
        if (categoryId)
            productData.categoryId = categoryId.trim();
        if (brandId)
            productData.brandId = brandId.trim();
        if (description)
            productData.description = description.trim();
        if (lowStockAlert !== undefined)
            productData.lowStockAlert = parseInt(lowStockAlert) || 10;
        if (internalSKU)
            productData.internalSKU = internalSKU.trim();
        if (vatPercentage !== undefined)
            productData.vatPercentage = parseFloat(vatPercentage) || 0;
        if (vatInclusive !== undefined)
            productData.vatInclusive = Boolean(vatInclusive);
        if (productionDate)
        ...
        if (discountRules)
            productData.discountRules = discountRules;
        ...
        if (showInQuickProducts !== undefined)
            productData.showInQuickProducts = Boolean(showInQuickProducts);
        const product = await Product.create(productData);
        // Invalidate cache for all barcodes (main + unit barcodes) to ensure fresh data
        await (0, productCache_1.invalidateAllProductBarcodeCaches)(storeId, product);
        res.status(201).json({
            success: true,
        ...
        logger_1.log.error('Error creating product', error, { storeId });
...
exports.getProducts = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        const limit = fetchAll
            ...
            : Math.min(100, Math.max(1, parseInt(req.query.limit) || 20)); // Max 100 items per page
        ...
        try {
            ...
            logger_1.log.error('Error counting products', countError, {
        ...
        try {
            ...
            logger_1.log.error('Error querying products', queryError, {
        ...
        if (includeCategories && products.length > 0) {
            try {
                ...
                logger_1.log.error('Error enriching products with categories', categoryError);
        ...
        logger_1.log.error('Error fetching products', error, {
            storeId: storeId,
            name: error.name,
            code: error.code,
        });
...
exports.getProduct = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const storeId = req.user?.storeId || null;
    if (!storeId) {
        return res.status(400).json({
    ...
    try {
        ...
        logger_1.log.error('Error fetching product', error);
        return res.status(500).json({
            success: false,
...
exports.updateProduct = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        // Use unified model with storeId filter for isolation
        const product = await Product.findOneAndUpdate({ _id: id, storeId: storeId.toLowerCase() }, updateData, { new: true, runValidators: true });
        if (!product) {
            return res.status(404).json({
                success: false,
                message: 'Product not found',
        ...
        if (oldProduct) {
            await (0, productCache_1.invalidateAllProductBarcodeCaches)(storeId, oldProduct);
        }
        ...
        logger_1.log.error('Error updating product', error);
        return res.status(500).json({
            success: false,
...
exports.deleteProduct = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        // Invalidate cache
        if (barcode) {
            await (0, productCache_1.invalidateProductCache)(storeId, barcode);
        }
        res.status(200).json({
            success: true,
        ...
        logger_1.log.error('Error deleting product', error);
        return res.status(500).json({
            success: false,
 ...
 */
exports.importProducts = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    if (!storeId && req.user?.userId && req.user.userId !== 'admin') {
        try {
            ...
            logger_1.log.error('Error fetching user', error);
        }
    }
    // Store users must have a storeId
    if (!storeId) {
    ...
    try {
        ...
        await (0, productCache_1.invalidateStoreProductCache)(storeId);
        const summary = {
        ...
        logger_1.log.error('Error importing products', error, {
            storeId: storeId,
        });
 ...
 */
exports.getProductMetrics = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    const storeId = req.user?.storeId || null;
    if (!storeId) {
        return res.status(400).json({
            success: false,
    ...
    try {
        ...
        logger_1.log.error('Error fetching product metrics', error);
        return res.status(500).json({
            success: false,
 ...
 */
exports.getProductByBarcode = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        const product = await (0, productCache_1.getProductByBarcode)(storeId, trimmedBarcode);
        if (!product) {
            return res.status(404).json({
                success: false,
                message: 'Product not found',
        ...
        logger_1.log.error('Error fetching product by barcode', error);
        return res.status(500).json({
            success: false,
