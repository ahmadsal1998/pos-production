...
const getProducts = (0, import_error.asyncHandler)(async (req, res) => {
  ...
  try {
    let Product;
    try {
      Product = await (0, import_productModel.getProductModelForStore)(storeId);
    } catch (modelError) {
    ...
    const fieldsToSelect = showInQuickProducts === "true" || showInQuickProducts === true ? "name price stock barcode showInQuickProducts status units costPrice categoryId brandId description updatedAt" : void 0;
    let products = [];
    try {
