...
const createWarehouse = (0, import_error.asyncHandler)(async (req, res, next) => {
  ...
  import_logger.log.debug("Create Warehouse - User info from token", {
  ...
  if (!storeId && req.user?.userId && req.user.userId !== "admin") {
    try {
      ...
      if (user && user.storeId) {
        ...
        import_logger.log.debug("Create Warehouse - Found storeId from user record", { storeId });
      }
    } catch (error) {
      import_logger.log.error("Create Warehouse - Error fetching user", error);
    }
  }
  if (!storeId) {
    import_logger.log.warn("Create Warehouse - No storeId found for user");
    return res.status(400).json({
      success: false,
  ...
  try {
    import_logger.log.debug("Create Warehouse - Getting Warehouse model for storeId", { storeId });
    let Warehouse;
    try {
      Warehouse = await (0, import_warehouseModel.getWarehouseModelForStore)(storeId);
      import_logger.log.debug("Create Warehouse - Warehouse model obtained");
    } catch (modelError) {
    ...
    import_logger.log.debug("Create Warehouse - Warehouse created successfully", { warehouseId: warehouse._id });
    res.status(201).json({
      success: true,
...
const getWarehouses = (0, import_error.asyncHandler)(async (req, res, next) => {
...
const getWarehouseById = (0, import_error.asyncHandler)(async (req, res, next) => {
  const { id } = req.params;
  const storeId = req.user?.storeId || null;
  if (!storeId) {
    return res.status(400).json({
...
const updateWarehouse = (0, import_error.asyncHandler)(async (req, res, next) => {
  const errors = (0, import_express_validator.validationResult)(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({
      success: false,
...
const deleteWarehouse = (0, import_error.asyncHandler)(async (req, res, next) => {
  const { id } = req.params;
  const storeId = req.user?.storeId || null;
  if (!storeId) {
    return res.status(400).json({
...
const exportWarehouses = (0, import_error.asyncHandler)(async (req, res, next) => {
  const storeId = req.user?.storeId || null;
  if (!storeId) {
    return res.status(400).json({
      success: false,
...
const importWarehouses = (0, import_error.asyncHandler)(async (req, res, next) => {
  const file = req.file;
  const storeId = req.user?.storeId || null;
  if (!file) {
    return res.status(400).json({
  ...
  try {
    for (let index = 0; index < normalizedRecords.length; index += 1) {
      ...
      const normalizedStoreId2 = storeId.toLowerCase().trim();
      let existing = await WarehouseModel.findOne({
        storeId: normalizedStoreId2,
        name
      });
      if (!existing) {
