...
async function generateNextInvoiceNumber(Sale, storeId) {
    ...
    try {
        ...
        let sequence = await Sequence_1.default.findOneAndUpdate({ storeId: normalizedStoreId, sequenceType }, { $inc: { value: 1 } }, { new: true });
        if (!sequence) {
            ...
            if (sequence.value <= maxExistingNumber) {
                ...
                sequence = await Sequence_1.default.findOneAndUpdate({ storeId: normalizedStoreId, sequenceType }, { $set: { value: maxExistingNumber + 1 } }, { new: true });
...
exports.createSale = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        const itemsHash = items
            .map((item) => `${item.productId || ''}-${item.productName || ''}-${item.quantity || 0}-${item.unitPrice || 0}`)
            .sort()
            .join('|');
        ...
        for (const recentSale of recentSales) {
            ...
            if (recentSale.items.length === items.length) {
                ...
                if (recentItemsHash === itemsHash &&
                    ...
                    logger_1.log.warn(`[Sales Controller] DUPLICATE INVOICE DETECTED: Invoice ${invoiceNumber} has same content as existing invoice ${recentSale.invoiceNumber} (created ${Math.round((Date.now() - new Date(recentSale.date).getTime()) / 1000)}s ago)`);
        ...
        logger_1.log.warn('[Sales Controller] Error checking for duplicate invoices, proceeding anyway:', duplicateCheckError);
    }
            ...
            if (existingSale) {
                ...
                if (existingSale.items.length === items.length) {
                    ...
                    if (existingItemsHash === incomingItemsHash &&
                        ...
                        logger_1.log.warn(`[Sales Controller] DUPLICATE INVOICE: Invoice ${currentInvoiceNumber} already exists with identical content`);
                        return res.status(409).json({
                            success: false,
                ...
                logger_1.log.warn(`[Sales Controller] Invoice number ${currentInvoiceNumber} already exists with different content. Generating new invoice number to avoid blocking.`);
