...
async function generateNextInvoiceNumber(Sale, storeId) {
  ...
  try {
    let sequence = await import_Sequence.default.findOneAndUpdate(
...
const createSale = (0, import_error.asyncHandler)(async (req, res) => {
  ...
  while (retryCount < maxRetries) {
    try {
      const existingSale2 = await Sale.findOne({
        invoiceNumber: currentInvoiceNumber,
        storeId: normalizedStoreId
      });
      if (existingSale2) {
        import_logger.log.warn(`[Sales Controller] Invoice number ${currentInvoiceNumber} already exists, generating new number`);
