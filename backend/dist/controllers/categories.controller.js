...
exports.createCategory = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    logger_1.log.debug('Create Category - User info from token', {
    ...
    if (!storeId && req.user?.userId && req.user.userId !== 'admin') {
        try {
            ...
            if (user && user.storeId) {
                ...
                logger_1.log.debug('Create Category - Found storeId from user record', { storeId });
            }
        }
        catch (error) {
            logger_1.log.error('Create Category - Error fetching user', error);
        }
    }
    // Store users must have a storeId
    if (!storeId) {
        logger_1.log.warn('Create Category - No storeId found for user');
        return res.status(400).json({
            success: false,
    ...
    try {
        ...
        logger_1.log.debug('Create Category - Category created successfully', { categoryId: category._id });
        res.status(201).json({
            success: true,
        ...
        logger_1.log.error('Create Category - Error', error, {
            stack: error.stack,
            storeId: storeId,
            name: error.name,
            code: error.code,
...
exports.getCategories = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        logger_1.log.error('Error fetching categories', error);
        return res.status(500).json({
            success: false,
...
exports.exportCategories = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        logger_1.log.error('Error exporting categories', error);
        return res.status(500).json({
            success: false,
...
exports.importCategories = (0, error_middleware_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        logger_1.log.error('Error importing categories', error);
        return res.status(500).json({
            success: false,
