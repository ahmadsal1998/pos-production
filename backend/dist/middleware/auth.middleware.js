...
const authenticate = async (req, res, next) => {
  try {
    ...
    if (isBarcodeRoute) {
      import_logger.log.debug("[Auth Middleware] BARCODE ROUTE - Authentication check", {
        path: req.path,
    ...
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      if (isBarcodeRoute) {
        import_logger.log.warn("[Auth Middleware] BARCODE ROUTE - Missing or invalid auth header");
      }
      res.status(401).json({
        success: false,
    ...
    try {
      ...
      req.user = decoded;
      if (isBarcodeRoute) {
        import_logger.log.debug("[Auth Middleware] BARCODE ROUTE - Token verified", {
      ...
      if (decoded.storeId && decoded.role !== "Admin") {
        try {
          ...
          if (!subscriptionStatus.isActive || subscriptionStatus.subscriptionExpired) {
            if (isBarcodeRoute) {
              import_logger.log.warn("[Auth Middleware] BARCODE ROUTE - Subscription expired");
            }
            res.status(403).json({
              success: false,
          ...
          import_logger.log.error(`Error checking subscription for store ${decoded.storeId}`, error);
        }
      }
      if (isBarcodeRoute) {
        import_logger.log.debug("[Auth Middleware] BARCODE ROUTE - Authentication passed, calling next()");
      }
      next();
    } catch (error) {
      const isBarcodeRoute2 = req.path.includes("/barcode") || req.originalUrl.includes("/barcode");
      import_logger.log.error("[Auth Middleware] Token verification failed", error, {
      ...
      if (isBarcodeRoute2) {
        import_logger.log.warn("[Auth Middleware] BARCODE ROUTE - Authentication failed");
      }
      res.status(401).json({
        success: false,
